  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div
          className={`px-4 py-2 bg-gradient-to-r ${currentTheme.primary} text-white rounded-lg shadow-lg`}
        >
        ...
        {dashboardData.metrics.map((metric, index) => {
          ...
          return (
            <div
              ...
              } border rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105`}
            >
              <div className="flex items-center justify-between mb-4">
                <div
                  className={`p-3 bg-gradient-to-r ${currentTheme.primary} rounded-lg shadow-md`}
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <div className="space-y-4">
            ...
            {/* Waiter Notification: Only show for waiter role */}
            {user?.role === "waiter" &&
              waiterNotifications.map((notif, idx) => (
                <div
                  key={idx}
                  className="flex items-center space-x-3 p-3 rounded-lg bg-blue-50 border-l-4 border-blue-400 dark:bg-blue-900 dark:border-blue-700 animate-pulse"
                >
                  <div className="w-2 h-2 rounded-full bg-blue-500"></div>
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-blue-900 dark:text-blue-200">
                      🪑 New Table Assigned: Table {notif.tableNumber} –{" "}
                      {notif.customerName} (Party of {notif.guests})
                    </p>
                    <p className="text-xs text-blue-700 dark:text-blue-300">
                      {notif.time} •{" "}
                      {notif.specialRequests &&
                        `Special: ${notif.specialRequests}`}
                    </p>
                  </div>
                </div>
              ))}
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {["cashier", "kitchen", "delivery", "customer"].includes(
              user?.role || ""
            ) &&
              [
